...
(async()=>{
 ...
 const newer=await call('POST',prefix+'/authorization-models',{schema_version:'1.1',type_definitions:[{type:'user'},{type:'team',relations:{member:{this:{}}},metadata:{relations:{member:{directly_related_user_types:[{type:'user'}]}}}}]});
 ...
 await page.locator('#models-tab').click(); await page.waitForFunction(()=>document.querySelectorAll('#model-select option').length===2);
 assert.equal(await page.locator('#model-select').inputValue(),newer.authorization_model_id);
 await page.locator('#model-select').selectOption(model.authorization_model_id);
 await page.locator('#tuples-tab').click(); await page.locator('#models-tab').click();
 assert.equal(await page.locator('#model-select').inputValue(),model.authorization_model_id);
 assert((await page.locator('#model-summary').innerText()).includes('3 types'));
